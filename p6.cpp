...
// undo within a level.
// You should store
// the old board,  -> class Board( declare this)
// the old playerToMove, -> 'X' or 'O'
// the old move number -> 1. Enter row, column, quadrant, direction for X:
...
void displayList( Node *pTemp)
{
    ...
    while (pTemp != nullptr){
        cout << pTemp->oldNum << ":" << pTemp->oldPiece;
        if (pTemp->pNext != nullptr){
            cout << "->";
        }
        pTemp = pTemp->pNext; // helps to go to next node
    }
    cout << endl;
} // end displayList()
...
void deleteNodeFromList(Node *&pHead,Board& theBoard, char &thePiece, int &Num )
{
    if (pHead->pNext == NULL){
...
void addNodeToList(Node *&pHead, Board theBoard, char thePiece, int Num)
{
    // Create a new node and store current values into it
    // ...
    Node *pTemp = new Node();
    pTemp -> oldBoard = theBoard;
    pTemp -> oldPiece = thePiece;
    pTemp -> oldNum = Num;
    pTemp -> pNext = pHead;
...
int main() {
    ...
    Row = ' ';
    Node *pHead = nullptr;
    addNodeToList(pHead,theBoard,piece1, moveNum);
    while(moveNum < 39) {
        ...
        if(Row == 'I'){
            ...
            deleteNodeFromList(pHead,theBoard,piece1,moveNum);
            continue;
        }
        ...
        moveNum++;
        addNodeToList(pHead,theBoard,piece2, moveNum);
    }
